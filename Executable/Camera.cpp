...
void CCamera::RegenerateViewMatrix() {
	...
	m_xmf3Right = Vector3::CrossProduct(m_xmf3Up, m_xmf3Look, true); //카메라의 z-축과 x-축에 수직인 벡터를 y-축으로 설정한다. 
...
CFirstPersonCamera::CFirstPersonCamera(CCamera *pCamera) : CCamera(pCamera) {
	...
	if (pCamera) { 
		...
		if (pCamera->GetMode() == SPACESHIP_CAMERA) { 
			m_xmf3Up = XMFLOAT3(0.0f, 1.0f, 0.0f); 
			m_xmf3Right.y = 0.0f; m_xmf3Look.y = 0.0f;
...
void CThirdPersonCamera::Update(XMFLOAT3& xmf3LookAt, float fTimeElapsed)
{
	...
	if (m_pPlayer) {
		...
		XMFLOAT3 xmf3Right = m_pPlayer->GetRightVector(); XMFLOAT3 xmf3Up = m_pPlayer->GetUpVector(); 
		XMFLOAT3 xmf3Look = m_pPlayer->GetLookVector(); 
